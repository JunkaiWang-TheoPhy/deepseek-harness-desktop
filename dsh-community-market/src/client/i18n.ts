...
export const MARKET_I18N = {
  'zh-CN': {
    ...
    'plugin.install': '安装',
    'plugin.detail': '详情',
    'install.lifecycle.warning': 'package 安装可能执行 lifecycle script',
    'install.confirm.title': '确认安装',
    'install.success': '安装成功，请重启 Desktop 加载新插件。',
    'install.error.identity-choice-required': '该插件同时声明 npm 包和仓库，请选择其中一种身份后再安装。',
    'sources.title': '来源管理',
  ...
  'en': {
    ...
    'install.lifecycle.warning': 'lifecycle scripts may run during installation',
    'install.confirm.title': 'Confirm installation',
    'install.success': 'Install succeeded. Restart Desktop to load the new plugin.',
    'install.error.identity-choice-required': 'This plugin declares both an npm package and a repository. Choose one before installing.',
