...
When the optional jobs service is composed, DSH Desktop also raises privacy-safe native attention for background jobs that finish with `completed` or `failed`. `killed` jobs stay silent. The live `dsh-desktop-job-notifications` settings namespace controls `notifyOnCompletion` and `notifyOnFailure`; both default to `true`. The native copy stays generic and never includes the job label, workspace paths, command text, session ids, or output.
