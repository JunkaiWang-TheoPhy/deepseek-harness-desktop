...
When composed with the optional jobs service and/or optional sessions service, DSH Desktop raises privacy-safe native attention for background jobs and for direct user turns that end at `turn/end`. Job notifications cover `completed` and `failed`, while `killed` jobs stay silent. Turn notifications cover `completed`, `error`, and `max-tokens`, while `aborted`, `blocked`, `interrupted`, mismatched turn endings, plugin-origin turns, continuation-only turns, and subagent sessions stay silent. The live `dsh-desktop-notifications` settings namespace controls `notifyOnTurnCompletion`, `notifyOnTurnFailure`, `notifyOnJobCompletion`, and `notifyOnJobFailure`; all default to `true`. The native copy stays generic and never includes prompt text, assistant text, session ids, paths, model or provider names, errors, tool data, job labels, commands, or output.
