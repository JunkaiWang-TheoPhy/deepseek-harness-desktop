...
当组合了可选 jobs service 时，DSH Desktop 还会为以 `completed` 或 `failed` 结束的后台任务触发保护隐私的原生 attention 提示；`killed` 任务保持静默。实时 `dsh-desktop-job-notifications` settings namespace 控制 `notifyOnCompletion` 与 `notifyOnFailure`，两者默认都为 `true`。原生文案始终保持泛化，不会包含任务 label、workspace 路径、命令文本、session id 或输出内容。
